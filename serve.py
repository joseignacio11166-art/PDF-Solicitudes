...
n8n llama: POST http://localhost:8000/fill-pdf
"""
...
# ─────────────────────────────────────────────
# SANITAS (PDF editable con pypdf)
# ─────────────────────────────────────────────
def fill_sanitas(data):
    ...
    direccion= data.get("direccion","Calle Hermosilla 80")
    numero   = data.get("numero","2A")
    cp       = data.get("cp","28001")
    ...
    fv = {
        ...
        "peso10": peso,
        "estatura10": altura,
        # Pregunta: ¿ex-Sanitas? → No (no marcar Sí_510)
        # Pregunta: ¿otra compañía? → No (no marcar Sí_610)
    ...
    # Tipo documento
    doc_map = {"Pasaporte": ["Pasaporte",    "Pasaporte_211"],
               "NIE":       ["NIE",          "NIE_210"],
               "NIF":       ["NIF",          "NIF_210"]}
    doc_fields = doc_map.get(tipo_doc, doc_map["Pasaporte"])
    for f in doc_fields:
        fv[f] = "/On"
    ...
    for page_idx in [0, 2, 3]:   # páginas con campos nuestros
...
def fill_nueva_mutua(data):
    ...
    # Convierte coordenada pdfplumber → ReportLab
    ...
    c.drawString(160, ry(147.6), fi_year)   # sobreescribe "20___"

    # ── SECCIÓN TOMADOR ──
    c.drawString(120, ry(186.6), nombre)
    c.drawString(73,  ry(200.6), num_doc)
    # DIRECCIÓN COMPLETA (fila vacía bajo el label, entre top=214.7 y top=234.9)
    ...
    # TELÉFONO MÓVIL (columna derecha)
    c.drawString(340, ry(271.6), telefono)

    # ── DIRECCIÓN PRESTACIÓN DEL SERVICIO (igual que tomador) ──
    ...
    # Sexo: marcamos "_X__" sobre el hueco
    if sexo in ("mujer","female","f","woman"):
        c.drawString(119, ry(509.8), "X")   # MUJER
    else:
        c.drawString(64,  ry(509.8), "X")   # HOMBRE
    ...
    if q1.lower() in ("sí","si","s","yes","y"):
        ...
        c.drawString(34, ry(746.6), "X")   # SI
    else:
        c.setFont("Helvetica-Bold", 9)
        c.drawString(64, ry(746.6), "X")   # NO

    # Detalle si afirmativo
...
if __name__ == "__main__":
    print("AlumnusCare PDF Server arrancando en http://localhost:8000")
