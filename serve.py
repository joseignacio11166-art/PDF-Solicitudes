"""
AlumnusCare PDF Fill Server — v6 DEFINITIVO
pypdf + set_need_appearances_writer(True) — probado y funciona perfecto
"""
...
NAC_MAP = {
    ...
    "italia":"Italiana","francia":"Francesa","alemania":"Alemana",
...
def fill_sanitas(data):
    # Fechas
    fn = (data.get("fecha_nac","1990-01-01")).split("-")
    fn_d, fn_m, fn_y = fn[2].zfill(2), fn[1].zfill(2), fn[0]
    fi = (data.get("fecha_inicio","01/01/2026")).split("/")
    fi_m, fi_y = fi[1].zfill(2), fi[2]
    ...
    nac      = NAC_MAP.get((data.get("nacionalidad","") or "").strip().lower(),
                           (data.get("nacionalidad","") or "").capitalize())
    ...
    # Pág 1
    s(1,"nombre tomador",nombre); s(1,"numero documento",num_doc)
    s(1,"dia2",fn_d); s(1,"mes2",fn_m); s(1,"año2",fn_y)
    s(1,"nacionalidad",nac); s(1,"movil1",tel); s(1,"email",email)
    s(1,"empresa",""); s(1,"domicilio tomador",dir_)
    s(1,"domicilio tomador n",num_v); s(1,"municipio tomador",mun)
    s(1,"cp tomador",cp); s(1,"provincia tomador",prov)
    ...
    # Pág 4 cuestionario
    s(4,"nombre asegurado pag310",nombre); s(4,"num doc10",num_doc)
    s(4,"día_410",fn_d); s(4,"mes_510",fn_m); s(4,"año_510",fn_y)
    s(4,"nacionalidado210",nac); s(4,"movil1 pag310",tel)
    s(4,"mes_610",fi_m); s(4,"año_610",fi_y)
    s(4,"parentesco10","el mismo"); s(4,"peso10",peso); s(4,"estatura10",altura)
    ...
    s(4,"No_530" if q3 in ("no","n") else "Sí_830","/On")
...
@app.route("/health")
def health():
    return jsonify({"status":"ok","version":"v6-definitivo"})
