...
- **📄 Solicitudes** (4 modos):
  ...
  - **✏️ Corregir un PDF** — REESCRITO jul 2026 con **VISIÓN**: la IA (`cerebro/vision.py`, Claude mirando el PDF como imagen JPEG) lee TODOS los datos aunque el PDF sea **escaneado/aplanado** (donde no hay texto), los revisas/corriges y se **REGENERA una solicitud limpia** (rellenar_nuevamutua / rellenar_sanitas), no se pinta encima. Esto arregla los 4 errores del método viejo (no detectaba imagen; sexo falso "Mujer" por leer la casilla ☐; el dato nuevo se solapaba con el viejo; la firma se cortaba). Firma: sin firma (por defecto) o recuperar la del original (extraer_firma → estampar en NM). Cuestionario de salud: la usuaria marca Sí/No por pregunta (soluciona el "sí tonto"). El viejo `core/corregir.py` (coordenadas) queda solo por `extraer_firma`.
